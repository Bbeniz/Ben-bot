narutogc.js const fs = require('fs');
  ...
  onStart: async function ({ api, event, args }) {
    ...
    try {
      ...
      if (participants.includes(event.senderID)) {
        ...
        api.sendMessage("📌 | t'as été ajouté au groupe ╔✇═❦═•| 🍀 |•═❦═✇╗ 🍀 Naruto  arcade 🍀 ╚✇═❦═•| 🍀 |•═❦═✇╝ ", event.threadID);

        // Set ✅ reaction for successfully added user
        api.setMessageReaction("🍀", event.messageID, "💌", api);
      }
    } catch (error) {
      api.sendMessage("❌ | Failed to add you to the group chat.\h the link:", event.threadID);
